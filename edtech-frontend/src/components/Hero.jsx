...
export default function Hero() {
  ...
  return (
    <section className="relative bg-gradient-to-br from-blue-700 to-blue-900 dark:from-gray-900 dark:to-black text-white py-24 overflow-hidden">

      {/* ✅ Animated Blob Background */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute w-96 h-96 bg-blue-500 opacity-20 blur-3xl rounded-full top-20 left-10 animate-pulse"></div>
        <div className="absolute w-72 h-72 bg-purple-500 opacity-20 blur-3xl rounded-full bottom-10 right-10 animate-pulse"></div>
      </div>

      <div className="max-w-6xl mx-auto px-6 text-center animate-fadeIn opacity-0" style={{ animationDelay: "0.2s" }}>
        ...
        <div
          ref={sliderRef}
          className="flex gap-6 overflow-x-auto no-scrollbar scroll-smooth mt-14 py-3"
        >
          {heroImages.map((img, i) => (
            <div
              key={i}
              className="flex-shrink-0 w-72 h-44 rounded-xl overflow-hidden shadow-xl hover:scale-105 transition transform duration-300"
            >
              <img
                src={img}
                alt="hero visuals"
